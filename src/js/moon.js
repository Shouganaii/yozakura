  ...
  class Scene {
    ...
    _buildBranch() {
      ...
      const grow = (x, y, dx, dy, len, thick, depth) => {
        ...
        for (let i = 0; i < kids; i++) {
          ...
          grow(px, py, nx / m, ny / m, len * (0.60 + rand() * 0.18), thick * 0.66, depth + 1);
        }
      };
      ...
      const trunk = [];
      {
        ...
        for (let k = 0; k < SEG; k++) {
          ...
          trunk.push({ x1: px, y1: py, x2: qx, y2: qy,
                       thick:  lerp(thick, endThick, k / SEG) * gnarl(k),
                       thick2: lerp(thick, endThick, (k + 1) / SEG) * gnarl(k + 1),
                       depth: 0, trunk: true });
          px = qx; py = qy;
        }
      ...
      for (const a of arms) {
        ...
        grow(x, y, a.dx / m, a.dy / m, a.len, a.thick, 1);
      }
    ...
    _blossomAt(b, p) {
      ...
      if (local <= 0) {
        ...
        return { x: hx + s, y: hy + s * 0.5, t: 0, landed: false };
      }
    ...
    _fillerAt(f, p) {
      ...
      if (t <= 0) {
        ...
        return { x: hx + s, y: hy + s * 0.5, alpha: 1 };
      }
    ...
    draw() {
      ...
      if (fg.alpha > 0.02) {
        ...
        const barkLit = shade(th.branch, 0.26);
        const pass = (widthK, offK, style) => {
          ...
          for (let i = 0; i < this.limbs.length; i += 40) {
            ...
            for (let k = i; k < end; k++) {
              ...
              let ax = fg.fx(L.x1) * this.vw + sway * this.vw * L.depth;
              let ay = fg.fy(L.y1) * this.vh;
              let bx = fg.fx(L.x2) * this.vw + sway * this.vw * (L.depth + 1);
              let by = fg.fy(L.y2) * this.vh;
              const dx = bx - ax, dy = by - ay;
              const len = Math.hypot(dx, dy) || 1;
              const ux = dx / len, uy = dy / len;
              const px = -uy, py = ux;
              const lit = px * -0.5 + py * -0.86;
              ...
              const o1 = L.thick * s * offK * 0.5 * lit, o2 = L.thick2 * s * offK * 0.5 * lit;
        ...
        /* Five steps rather than three: at three the trunk read as a pole
         * painted in three stripes rather than a round thing catching light. */
        pass(1.00, 0.00, barkDark);
        pass(0.84, 0.10, mixHex(barkDark, th.branch, 0.55));
        pass(0.63, 0.20, th.branch);
        pass(0.41, 0.29, mixHex(th.branch, barkLit, 0.55));
        pass(0.20, 0.36, barkLit);
        g.globalAlpha = 1;
      }
